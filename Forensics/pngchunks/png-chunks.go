...
var (
	...
	filename                string
	chunkFlag               bool
	includeSingleLengthSigs bool
)

func init() {
	// Setup flags.
	flag.StringVar(&filename, "file", "", "Input file")
	flag.BoolVar(&chunkFlag, "c", false, "Print chunk info for all chunks.")
	flag.BoolVar(&chunkFlag, "chunks", false, "Print chunk info for all chunks.")
	flag.BoolVar(&includeSingleLengthSigs, "includeSingleByteSignatures", false, "Includes file signatures of length 1 byte for consideration.")
...
func main() {
	...
	highlightedChunks := make([]Highlight, 0)
	for i, c := range png.chunks {
		for _, sig := range signatures {
			if !includeSingleLengthSigs && len(sig.Sig) == 1 {
				continue
			}
			if len(c.Data) < len(sig.Sig) {
				continue
			}
			if bytes.Equal(c.Data[:len(sig.Sig)], sig.Sig) {
				highlightedChunks = append(highlightedChunks, Highlight{Num: i, C: *c, Sig: sig})
			}
		}
	}

	for _, c := range highlightedChunks {
		fmt.Printf("Found Chunk %d with sig %x : %s.\n", c.Num, c.Sig.Sig, c.Sig.Filename)
	}
	if len(highlightedChunks) == 0 {
		fmt.Println("No file signatures found.")
	}
